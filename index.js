...
urlSchema.pre("save", function(next) {
    ...
    Counter.findByIdAndUpdate(
        ...
        function(err, counter) {
            if (err) {
                console.error(
                    "APP: Error while finding and updating counter value"
                );
                return next(err);
            }
...
promise = mongoose.connect(connectionString, { useNewUrlParser: true });
...
promise.then(function(db) {
    ...
    Counter.remove({}, function() {
        ...
        console.log(
            "APP: Initializing Counter collection with a default value"
        );
    ...
    if (baseid) {
        ...
        URL.findOne({ _id: id }, function(err, doc) {
            if (doc) {
                ...
                console.log(
                    "APP: Could not find ID in DB, redirecting to home"
                );
                res.redirect("/");
            }
        });
